...
INGRID=$(cat $ACCOUNTS_DIR/bob.txt | awk '/testnet/ { count++; if (count == 1) print $2}')
...
fund_accounts() {
  echo "Funding accounts for Alice, Ingrid and Bob with SUDT tokens"
  SUDT_AMOUNT=100000000

  expect << EOF
  spawn ckb-cli sudt issue --owner $GENESIS --udt-to $ALICE:$SUDT_AMOUNT $INGRID:$SUDT_AMOUNT $BOB:$SUDT_AMOUNT --cell-deps $SYSTEM_SCRIPTS_DIR/sudt-celldep.json
...
list_accounts_balances() {
  echo "Listing SUDT account balances for Alice, Ingrid and Bob"
  echo "ALICE: ========================================"
